...
use crate::interactive_pickers::{
    BranchPicker, ModelEntry, ModelPicker as PickerModelPicker, SessionPicker,
};
...
// Split out for size (P7, `docs/tui-pending-action-plan.md`): each is an
// `impl InteractiveMode { .. }` block of inherent methods, exactly as
// callable from here as if still defined in this file — Rust does not care
// which file in a crate defines a method for `self.method()` call sites to
// resolve it.
mod commands;
mod pickers;
